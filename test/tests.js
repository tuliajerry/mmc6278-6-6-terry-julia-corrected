    ...
    describe("GET /cart - cart page", () => {
      ...
      for (const {
        ...
        describe(`Cart Item: ${name}`, () => {
          ...
          it(`should render link to product`, async () => {
            const res = await request(app).get("/cart");
            expect(res.status).to.eq(200);
            expect(res.headers["content-type"]).to.include("html");
            const {
            ...
            const nameEl = getByText(document, name);
            const link = nameEl.closest(".cart-item-info").closest('a')
            expect(link).to.exist
            expect(link.href).to.eq('/product/' + inventoryId)
          });
          ...
          it(`should render cart id on input`, async () => {
            const res = await request(app).get("/cart");
            expect(res.status).to.eq(200);
            expect(res.headers["content-type"]).to.include("html");
            const {
            ...
            const input = getByLabelText(itemContainer, /quantity/i);
            expect(input).to.exist;
            const [[{id}]] = await db.query(`SELECT id FROM cart WHERE inventory_id=?`, inventoryId)
            expect(input.dataset.cartId).to.eq(id.toString())
          });
